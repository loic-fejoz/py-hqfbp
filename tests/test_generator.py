...
def test_generator_announcement():
    ...
    gen = PDUGenerator(
        ...
        encodings=[
            ...
            "h",
            "crc32",
        ],  # Not realistic as CRC32 is still transparent but test the principle
...
def test_generator_parse_encodings():
    ...
    assert gen._parse_encodings(["gzip", "h", "aes", "crc32"]) == (
        ["gzip"],
        ["aes", "crc32"],
        True,
    )
...
def test_generator_rq_encoding():
    ...
    repair_count = 2
    gen = PDUGenerator(
        src_callsign="F4JXQ", encodings=[f"rq({len(data)},{mtu},{repair_count}),h"]
    )
    pdus = list(gen.generate(data))
    ...
    assert header[HQFBP_CBOR_KEYS["Content-Encoding"]] == [
        f"rq({len(data)},{mtu},{repair_count})",
        -1,
    ]
...
def test_generator_rq_encoding_with_autolen():
    ...
    repair_count = 2
    gen = PDUGenerator(
        src_callsign="F4JXQ", encodings=[f"rq(dlen,{mtu},{repair_count}),h"]
    )
    pdus = list(gen.generate(data))
...
def test_generator_rq_post_boundary():
    ...
    repair_count = 4
    gen = PDUGenerator(
        src_callsign="F4JXQ", encodings=["h", f"rq({rq_len},{mtu},{repair_count})"]
    )
...
def test_generator_rq_post_boundary_autolen():
    ...
    gen = PDUGenerator(
        src_callsign="F4JXQ", encodings=["h", f"rq(dlen,{mtu},{repair_count})"]
    )
