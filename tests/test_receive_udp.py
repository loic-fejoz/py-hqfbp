...
class TestCLIUDP(unittest.TestCase):
    ...
    def test_send_success(
        self, mock_isfile, mock_file, mock_guess, mock_socket, mock_gen
    ):
    ...
    def test_receive_success(
        ...
        mock_msg_ev = MessageEvent(
            header={
                HQFBP_CBOR_KEYS["Src-Callsign"]: "F4JXQ",
                HQFBP_CBOR_KEYS["Content-Type"]: "text/plain",
            },
            payload=b"full message",
        )
        mock_deframer_instance.next_event.side_effect = [
            mock_pdu_ev,
            mock_msg_ev,
            None,
            None,
        ...
        with patch.object(sys, "argv", test_args):
            with patch("sys.stdout", new_callable=MagicMock()) as mock_stdout:
                ...
                found_ann = any(
                    "📢 Announcement for Msg-Id 123" in str(call)
