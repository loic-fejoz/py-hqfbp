...
class Deframer:
    ...
    def _is_fragmented(self, header: Dict[int, Any], payload_len: int) -> bool:
        ...
        if not has_h: return False
        has_reassembly = any(isinstance(e, str) and (CHUNK_RE.match(e) or REPEAT_RE.match(e)) for e in post)
        if has_reassembly:
            expected = header.get(HQFBP_CBOR_KEYS["Payload-Size"])
            if expected is not None: return payload_len < expected
        return False

    def _apply_pdu_level_decodings(self, header: Dict[int, Any], payload: bytes) -> Tuple[bytes, int]:
        ce = header.get(HQFBP_CBOR_KEYS["Content-Encoding"])
        if not ce: return payload, 0
    ...
    def receive_bytes(self, data: bytes):
        ...
        if header is None or msg_id is None:
            for ann_encs in self._announcements.values():
                ...
                try:
                    stripped, q = self._strip_post_boundary_encodings(try_list, ann_encs)
                    h2, p2 = unpack(stripped)
                    if self._is_fragmented(h2, len(p2)): continue
        ...
        is_ann = (
            (cont_type is not None and cont_type == "application/vnd.hqfbp+cbor") or
            (cont_fmt is not None and cont_fmt == COAP_CONTENT_FORMATS.get("application/vnd.hqfbp+cbor"))
        )
        ...
        if is_ann:
            ...
            if not decoded_pdu_level:
                ...
                if ae: a_p, _ = self._apply_decoding_list(a_p, self._split_encodings(ae)[0], True)
        ...
        if session_key not in self._sessions:
            self._sessions[session_key] = {'chunks': {}, 'headers': [], 'total_chunks': header.get(HQFBP_CBOR_KEYS["Total-Chunks"], 1)}
        ...
        completed = False
        if len(session['chunks']) == session['total_chunks']: completed = self._complete_message(session_key)
        else:
            rq = self._get_rq_info(session['headers'])
            if rq and len(session['chunks']) >= ((rq[0] + rq[1] - 1) // rq[1]): completed = self._complete_message(session_key)
        
        if completed:
            self._announcements.pop((src_callsign, msg_id), None)
            if orig_id != msg_id: self._announcements.pop((src_callsign, orig_id), None)
    ...
    def _handle_announcement(self, src_callsign: Optional[str], payload: bytes):
        try:
            ...
            tid, tce = ann_data.get(HQFBP_CBOR_KEYS["Message-Id"]), ann_data.get(HQFBP_CBOR_KEYS["Content-Encoding"])
            if tid is not None: self._announcements[(src_callsign, tid)] = tce if isinstance(tce, list) else [tce]
        except Exception: pass

    def _apply_decoding_list(self, data: Union[bytes, List[bytes]], encodings: List[Union[int, str]], pre_boundary: bool) -> Tuple[bytes, int]:
        quality = 0
            ...
            if enc in (1, "gzip", 3, "br", 4, "lzma", 5, 6, "crc16", "crc32"):
                ...
                if m_rep:
                    ...
                    if isinstance(data, list): data = data[::count]
                        ...
                        if block_len > 0: data = data[:block_len]
                elif m_chunk:
                    if isinstance(data, list): data = b"".join(data)
                elif m_rs:
                    if isinstance(data, list): data = b"".join(data)
    ...
    def _strip_post_boundary_encodings(self, data: Union[bytes, List[bytes]], encodings: List[Union[int, str]]) -> Tuple[Union[bytes, List[bytes]], int]:
    ...
    def _complete_message(self, session_key: Tuple[Optional[str], int]) -> bool:
        session = self._sessions[session_key]
        chunks = [session['chunks'].get(i)[0] for i in range(session['total_chunks']) if session['chunks'].get(i)]
        ...
        try:
            ...
            try:
                ...
                if h_i and h_i.get(HQFBP_CBOR_KEYS["Message-Id"]) is not None:
                    pre_i, _, _ = self._split_encodings(h_i.get(HQFBP_CBOR_KEYS["Content-Encoding"]))
                    if pre_i: p_i, _ = self._apply_decoding_list(p_i, pre_i, True)
                    merged_header, full_payload = h_i, p_i
            except Exception: pass
            sz = merged_header.get(HQFBP_CBOR_KEYS["File-Size"])
            if sz is not None: full_payload = full_payload[:sz]
    ...
    def _split_encodings(self, encodings: Union[int, str, List[Union[int, str]]]) -> Tuple[List[Union[int, str]], List[Union[int, str]], bool]:
        if encodings is None: return [], [], False
        encs = encodings if isinstance(encodings, list) else [encodings]
        for i, e in enumerate(encs):
            if e in (-1, "h"): return encs[:i], encs[i+1:], True
        return encs, [], False
