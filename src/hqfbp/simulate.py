...
class SimulationMetrics:
    ...
    def add_residual_errors(self, original_payload: bytes, decoded_payload: bytes):
        ...
        self.total_residual_bit_errors += (
            abs(len(original_payload) - len(decoded_payload)) * 8
        )

    def report(self, format="markdown") -> str:
        efficiency = (
            (self.total_payload_bits / self.total_bits_sent * 100)
            if self.total_bits_sent > 0
            else 0
        )
        packet_loss_rate = (
            (self.pdus_lost / self.total_pdus_sent * 100)
            if self.total_pdus_sent > 0
            else 0
        )
        file_loss_rate = (
            ((self.files_attempted - self.files_recovered) / self.files_attempted * 100)
            if self.files_attempted > 0
            else 0
        )
        overhead = (
            ((self.header_bits + self.padding_bits) / self.total_bits_sent * 100)
            if self.total_bits_sent > 0
            else 0
        )
        fec_recovery = (
            (self.files_recovered / self.files_attempted * 100)
            if self.files_attempted > 0
            else 0
        )
        rber = (
            (self.total_residual_bit_errors / self.total_bits_evaluated)
            if self.total_bits_evaluated > 0
            else 0
        )
        air_ber = (
            (self.total_bit_errors_introduced / self.total_bits_on_air)
            if self.total_bits_on_air > 0
            else 0
        )
...
def simulate(
    ber: float, encodings: str, ann_encodings: Optional[str], file_size: int, limit: int
):
            ...
            try:
                ...
                while True:
                    ...
                    if isinstance(ev, PDUEvent):
                        ...
                        if ev.payload.startswith(
                            source_data
                        ):  # Because some padding may have been added.
                            recovered = True

                metrics.add_pdu(
                    clean_pdu, lost=not pdu_accepted, errors_in_pdu=errors_in_pdu
                )
...
def main():
    ...
    parser.add_argument(
        "--ann-encodings", type=str, default=None, help="Announcement encodings"
    )
    parser.add_argument(
        "--file-size", type=int, default=1024, help="File size in bytes"
    )
    parser.add_argument(
        "--limit", type=int, default=10, help="Number of files to transmit"
    )
    parser.add_argument(
        "--format",
    ...
    metrics = simulate(
        args.ber, args.encodings, args.ann_encodings, args.file_size, args.limit
    )
