...
class PDUGenerator:
    ...
    def _apply_encodings(
        self, data: bytes, encodings: List[Union[str, int]]
    ...
    def _parse_encodings(
        self, val: Union[str, List[Union[str, int]], None]
    ) -> Tuple[List[Union[str, int]], List[Union[str, int]], bool]:
            ...
            for i, e in enumerate(pre):
                if isinstance(e, str):
                    ...
                    if rq_m:
                        ...
                        has_chunk = (
                            True  # Mark as handled to avoid max_payload_size chunking
                        )
                        break
    ...
    def generate(
        self, data: bytes, content_type: Optional[str] = None
        ...
        for enc_idx, enc in enumerate(full_encs):
            if enc in (-1, "h"):
                ...
                for i, chunk_data in enumerate(current_chunks):
                    ...
                    header[HQFBP_CBOR_KEYS["Content-Encoding"]] = self._clean_encodings(
                        full_encs
                    )
                ...
                for c in current_chunks:
                    ...
                    if isinstance(chunks, bytes):
                        ...
                        raise TypeError(
                            f"Unexpected return type from _apply_encodings: {type(chunks)}"
                        )
                current_chunks = new_chunks
        ...
        if self.announcement_encoder:
            ...
            ann_payload_dict[HQFBP_CBOR_KEYS["Content-Encoding"]] = (
                self._clean_encodings(full_encs)
            )

            for pdu in self.announcement_encoder.generate(
                pack(ann_payload_dict, b""), content_type="application/vnd.hqfbp+cbor"
            ):
                yield pdu
